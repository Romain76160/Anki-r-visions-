...
function home(){
  ...
  const c=counts();return`${renderTop('MOOC Révision','Répétition espacée FSRS-6')}<div class="panel hero"><h1>${c.due?`${c.due} carte${c.due>1?'s':''} à réviser`:'Tout est à jour'}</h1><p>${c.due?'Tes cartes dues et nouvelles sont prêtes.':'Aucune carte n’est due pour le moment.'}</p><div class="stats"><div class="stat"><b>${c.due}</b><span>À revoir</span></div><div class="stat"><b>${c.newc}</b><span>Nouvelles</span></div><div class="stat"><b>${c.learned}</b><span>Apprises</span></div></div><div style="height:16px"></div><button class="full" id="startStudy" ${c.due?'':'disabled'}>${c.due?'Commencer':'Rien à réviser'}</button></div><div class="section-title">Principe</div><div class="panel"><b>FSRS adapte chaque intervalle à ta mémoire.</b><p class="muted">Encore = oubli. Difficile, Bien et Facile indiquent des niveaux de rappel différents.</p></div>${isIOS()&&!isStandalone()?installPanel():''}`;}
function study(){const c=state.session[state.index];if(!c)return home();const ints=[1,2,3,4].map(g=>reviewCalc(progressFor(c.id),g).interval);return`${renderTop(`Révision ${state.index+1}/${state.session.length}`)}<div class="card study-card"><div class="row spread"><span class="subject-pill">${esc(c.subject)}</span><span class="muted">${esc(c.chapter||'')}</span></div>${!state.revealed?`<h2>${esc(c.prompt)}</h2><button id="reveal" class="secondary full">Afficher la réponse</button>`:`<div class="answer"><b>${esc(c.answer)}</b>${c.explanation?`<div class="explanation">${esc(c.explanation)}</div>`:''}</div><div class="grade-grid"><button class="grade again" data-grade="1">Encore<small>${formatInterval(ints[0])}</small></button><button class="grade hard" data-grade="2">Difficile<small>${formatInterval(ints[1])}</small></button><button class="grade good" data-grade="3">Bien<small>${formatInterval(ints[2])}</small></button><button class="grade easy" data-grade="4">Facile<small>${formatInterval(ints[3])}</small></button></div>`}</div>`;}
